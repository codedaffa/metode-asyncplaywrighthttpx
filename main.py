...
async def main(headers, keyword, pages):
    ...
    async with httpx.AsyncClient() as session:
        ...
        semaphore = asyncio.Semaphore(int(chunk_size/2))
        ...
        for i in range(chunk_size):
            tasks.append(
                asyncio.create_task(
                    scrape_produk(product_soup_chunks[i], headers, session, semaphore)
                )
            )
...
async def scrape_produk(product_soup, headers, session, semaphore):
    async with semaphore:
